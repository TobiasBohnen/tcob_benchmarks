...
{
    auto constexpr getBox = +[](Node* node) { return node->box; };
    ...
    for (auto _ : state) {
        ...
        auto quadtree      = tcob::gfx::quadtree<Node*, getBox>(box);
...
void QUADTREE_Query4(benchmark::State& state)
{
    ...
    auto constexpr getBox = [](Node* node) {
        return node->box;
    };
    ...
    for (auto _ : state) {
        auto intersections = std::vector<std::vector<Node*>>(nodes.size());
        auto quadtree      = tcob::gfx::quadtree<Node*, getBox, 4>(box);
        for (auto& node : nodes) { quadtree.add(&node); }
...
void QUADTREE_Query16(benchmark::State& state)
{
    ...
    for (auto _ : state) {
        auto intersections = std::vector<std::vector<Node*>>(nodes.size());
        auto quadtree      = tcob::gfx::quadtree<Node*, getBox, 16>(box);
        for (auto& node : nodes) { quadtree.add(&node); }
        for (auto const& node : nodes) { intersections[node.id] = quadtree.query(node.box); }
    }
}
